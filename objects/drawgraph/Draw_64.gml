...
if go == 4 or go == 4.1
{count = count + 1
	if count >= 75 && count < 100 {draw_sprite_ext(rushd6,0,global.guiw * 0.5 + textr, global.guih * 0.5 + texty,6,6,0,c_white,trans) audio_play_sound(snd_rushscream,6,false)}
	...
	if count >= 100 {go = go + 1}
		...
		if go == 5 or go == 5.1
		{draw_sprite_ext(rushd6,0,global.guiw * 0.5, global.guih * 0.5,6 + ra,6 + ra,0,c_white,1) if go == 5 {audio_play_sound(snd_rushscreamt,6,true,-100) go = go + 0.1}
			
		}
		if go == 6
		{
			audio_stop_sound(snd_rushscreamt)	
		}
